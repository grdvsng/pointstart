...
Func _PreSetting()
   ...
   $bLoad = GUICtrlCreateButton("Select a file", $left, $iTop, $bweight, $bheight, 0x0001)
   $InpName = GUICtrlCreateInput("User name", $left, $top+$iTop, $bweight, $bheight*0.5, $ES_AUTOHSCROLL)
   $ButName = GUICtrlCreateButton("Confirm", $left, $top+($bheight*0.5)+$iTop, $bweight, $bheight*0.5, 0x0300 + 0x0001)
   $InpPass = GUICtrlCreateInput("Password", $left, ($top*2)+$iTop, $bweight, $bheight*0.5, BitOR($ES_READONLY, $ES_CENTER))
   $ButPass = GUICtrlCreateButton("Confirm", $left, $top*2+($bheight*0.5)+$iTop, $bweight, $bheight*0.5, 0x0300 + 0x0001)
   $bCompile = GUICtrlCreateButton("Generate link", $left, ($top*3)+$iTop, $bweight, $bheight, 0x0300 + 0x0001)

   ; Специальные надстройки для 'инпутов'.
   _InputColor($InpPass)
   _InputColor($InpName)
   GUICtrlSetTip($InpName, "Enter the data in the format: domain \ username. If the user is local, enter only the username.", "Information")
EndFunc
   ...
   While 0 < 1
	  ...
	  If @error = 0 Then
		 ...
		 Case $CurInfo[4] = $InpPass And $CurInfo[2] = 1 And $truepass == ""
			   $truepass = InputBox("Enter data", "User password for which label is generated.", "", "*")

			   If $truepass <> "" Then
				  GUICtrlSetData($InpPass, "Data entered")
				  GUICtrlSetBkColor($InpPass, 0xFFFF00)
			   EndIf

		 Case $CurInfo[4] = $InpName And $CurInfo[2] = 1 And  ControlGetText('', '', $InpName) == "User name"
