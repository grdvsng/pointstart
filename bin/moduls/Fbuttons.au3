...
Func _form($but, $inp, $opt=0)
   ...
   If $Data == 'Подтвердить' Then
	  If $BoxReturn = 6 Then
			GUICtrlSetBkColor($inp, 0xD7FED7)
			GUICtrlSetData($but, 'Отмена')
   ...
   ElseIf $Data == 'Отмена'  Then
	  If $BoxReturn = 6 Then
			if $inp = $InpName Then
			   GUICtrlSetData($inp, 'Имя Пользователя')
			Else
			   GUICtrlSetStyle($inp, $ES_READONLY)
			   GUICtrlSetData($inp, 'Пароль')
			   $truepass = ""
			EndIf

			GUICtrlSetBkColor($inp, 0xFFFFFF)
			GUICtrlSetBkColor($but, $butscolor)
			GUICtrlSetData($but, 'Подтвердить')
...
Func _Load($opt=0)
   ...
   If $opt = 0 Then $val = FileOpenDialog("Выберите файл", @ProgramFilesDir, "Приложение(*.exe)", 2)
   If $opt = 1 Then $val=''
...
Func _close($e=777)
   ...
   #ce
   Dim $errors[10]
   $errors[0]='#0 Файл с надстройками не найден.'
   $errors[1]='#1 Файлы реестра повреждены или не найдены, попробуйте перезапустить программу от имени администратора.'
   $errors[2] = '#2 Ошибка при обращении к программе. '
   $errors[2] &= 'Вероятно введенные данные некорректны. Попробуйте сгенерировать ссылку заново.'
   $errors[3] = "#3 Файл изменен или несовместим с программой, запуск невозможен, сгенирируйте ярлык заново."
   $errors[4] = "#4 Для генерации ярлыка необходимы права Администратора."
   $errors[5] = "#5 Ошибка при записи данных регистор, продолжение невозможно."
   $errors[6] = "#6 Ненайдет вспомогательный модуль name."

   if $e < 777 Then
	  $txt = 'Приложение было остановлено в связи с ошибкой: ' & @CR & $errors[$e]
	  MsgBox(0, 'Внимание!', $txt)
   EndIf
