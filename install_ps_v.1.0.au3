...
$txt &= @CR
$txt &= @CR & 'Вас приветствует установка'
$txt &= @CR & 'программы «PointStart».'
$txt &= @CR & @CR & 'Автор программы: grdvsng@gmail.com'
...
Func Final($exe, $ico='')
   ...
   $request=MsgBox(4, 'Программа установлена!', 'Добавить ярлык на рабочий стол?')
   If $request=6 Then dirtorion($exe, $ico)
   BP()
   MsgBox(0, "Внимание!", "Установка успешно завершена.")
   Exit
...
Func SetGui()
   Local  $version = StringSplit(FileGetVersion($fPath), ".", 2), _
	      ...
	      $folider = GUICtrlCreateButton("Обзор", $w-($wbut*1.1), $h-($wbut*0.9), $wbut, $whight), _
   ...
   If $last <> '' Then
	  Select
	  Case $myvers > $last
		 MsgBox(4, 'Внимание!', 'Найдена более старая версия файла, хотите обновить и продолжить?')

	  Case $myvers = $last
		 $iBox = MsgBox(4, 'Внимание!', 'На машине уже установлена текущая версия, хотите продолжить (существующее приложение будет удалено)?')

      Case Else
		 $iBox  MsgBox(4, 'Внимание!', 'На машине уже установлена обновленная версия, хотите продолжить установку (существующее приложение будет удалено)?')
	  EndSelect
   ...
   While 1
	  ...
	  Case $folider
		 $dir = FileSelectFolder('Выберите директорию для установки', @ProgramFilesDir)
...
Func IfExist()
   $request=MsgBox(4, 'Внимание', 'Путь назначен, хотите продолжить?')
...
Func Error($rise, $box=5)
   ...
   $resolution = MsgBox($box, 'Внимание!','Возникла ошибка: ' & $rise)
...
Func erros_catcher($func)
   Execute($func)
   if @error <> 0 Then Error('не удалось удалить ' & $func & @CRLF & 'Продолжить?', 4)
EndFunc
